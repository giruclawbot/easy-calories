...
export function EditMealModal({ meal, onSave, onClose }: Props) {
  ...
  // Original nutrition was stored per-quantity (not per 100g), so we need per-unit values
  // We recalculate from original: originalPerUnit = original / original_quantity
    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4" role="dialog" aria-modal="true" aria-labelledby="edit-title">
      <div className="bg-gray-900 rounded-2xl w-full max-w-sm border border-gray-800">
        <div className="flex items-center justify-between p-5 border-b border-gray-800">
          <h2 id="edit-title" className="font-bold text-white">Editar porción</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-white text-xl" aria-label="Cerrar">×</button>
        </div>

        <form onSubmit={handleSubmit(onSubmit)} className="p-5 space-y-4">
          <div>
            <p className="text-white font-medium text-sm">{meal.foodName}</p>
            <p className="text-gray-500 text-xs mt-0.5">Ajusta la cantidad</p>
          </div>
          ...
          {preview && (
            <div className="grid grid-cols-4 gap-1.5 text-center text-xs">
              {[
                { label: 'Proteína', value: preview.protein, color: 'text-blue-400' },
                { label: 'Carbs', value: preview.carbs, color: 'text-yellow-400' },
                { label: 'Grasa', value: preview.fat, color: 'text-orange-400' },
                { label: 'Fibra', value: preview.fiber, color: 'text-green-400' },
              ].map(m => (
